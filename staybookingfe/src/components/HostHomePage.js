...
class MyStays extends React.Component {
    ...
    loadData = async () => {
      ...
      try {
        // await will parse info in Promise returned from backend
        // if we use ().then((resp) => { })
    ...
    render() {
      return (
        <List
          ...
          // instruct List what to do with data
          // item is from dataSource
          renderItem={(item) => (
            <List.Item>
              <Card
                key={item.id} // string does not need{}, anything else does
                title={
              ...
              >
                {
                  ...
                  >
                    {item.images.map((image, index) => (
                      <div key={index}>
                        <Image height={200} src={image.url} width="100%" /> 
                      </div>
                    ))}
                  </Carousel>
                }
...
export class StayDetailInfoButton extends React.Component {
    ...
    render() {
        const { stay } = this.props;    // props passed in from stay class
...
class RemoveStayButton extends React.Component {
    state = {
      ...
      handleRemoveStay = async () => {
        const { stay, onRemoveSuccess } = this.props;   // onRemoveSuccess = loadData function is call back function passed in from MyStays
        this.setState({
          loading: true,
        });
     
        try {
          await deleteStay(stay.id); // if delete stay fails, will not call onRemoveSuccess
          onRemoveSuccess();    // call back function will be used once we delete stay
        } catch (error) {
          message.error(error.message);
        } finally {
          this.setState({
...
class ViewReservationsButton extends React.Component {
  ...
  render() {
    ...
    const modalTitle = `Reservations of ${stay.name}`;  // ``build string template
...
// integrates function call to check reservation status, only calls this API when button clicked
// different from logic of detailed info button
class ReservationList extends React.Component {
  ...
  loadData = async () => {
    ...
    try {
      ...
      this.setState({     // assign resp from backend to reservation field in state
        reservations: resp,
      });
    } catch (error) {
      message.error(error.message);
